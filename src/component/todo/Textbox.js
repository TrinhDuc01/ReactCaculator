...
const Textbox = () => {
    ...
    const [todoList, setTodoList] = useState([{ Todo: "Quét nhà" }, { Todo: "Làm bài tập" }]);
    ...
    return (
        <div>
            <input className="inputTodo" value={todo} onChange={(e) => handleTodo(e)} />
            {buttonSwitch ? <button className="buttonTodo" onClick={() => handleSaveTodo()}>Thêm</button> : <button className="buttonTodo" onClick={handleFixTodo}>Sửa</button>}
            <TodoList
                todoList={todoList}
